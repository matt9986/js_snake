...
describe("curry", function () {
    ...
    it("creates re-useable methods", function () {
        const func = (first, second) => first + second;
        const curried = curry(func)("win");
        assert.equal(curried("big"), "winbig");
        assert.equal(curried("fast"), "winfast");
        assert.equal(curried("smart"), "winsmart");
    })
});
